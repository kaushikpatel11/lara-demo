    ...
    <table cellspacing="0" cellpadding="0px" width="600px" align="center" style="background-color: #080522;">
        ...
        <tr>
            <td style="padding:20px 30px;min-height:300px; vertical-align: top;">
                <h3 style="font-size: 18px;margin-bottom: 10px; color:#ffffff">Hey {{$ReqData->agentBand->createdBy->fname}} {{$ReqData->agentBand->createdBy->lname}}</h3>
                <p style="color:#ffffff">{{$user['fname']}}  {{$user['lname']}} has requested management access to {{$ReqData->agentBand->name}}'s demo profile.</p>
                <p style="margin-top:60px;margin-bottom: 40px;">
                   <a href="{{ url(env('demo_WEBSITE_URL')) .'/update/request-status'. '?confirmation_token='.urlencode($user->api_token).'&band_token='.urlencode($ReqData->band_api_token).'&status='.urlencode('ACCEPTED_BY_TALENT') }}" style="background-color: #64FECC; border:2px solid #64FECC; color: #0F24FD; border-radius: 3px; font-weight: bold; font-size: 15px; text-decoration: none; padding:12px 20px; box-shadow: rgba(50, 50, 93, 0.25) 0px 13px 27px -5px, rgba(0, 0, 0, 0.3) 0px 8px 16px -8px; margin-right: 15px">Approve</a>
                    <a href="{{ url(env('demo_WEBSITE_URL')) .'/update/request-status'. '?confirmation_token='.urlencode($user->api_token).'&band_token='.urlencode($ReqData->band_api_token).'&status='.urlencode('REJECTED_BY_TALENT') }}" style="background-color: transparent; border:2px solid #64FECC; color: #64FECC; border-radius: 3px; font-weight: bold; font-size: 15px; text-decoration: none; padding:12px 20px; box-shadow: rgba(50, 50, 93, 0.25) 0px 13px 27px -5px, rgba(0, 0, 0, 0.3) 0px 8px 16px -8px;">Reject</a>
                </p>
            </td>
        </tr>
        <!-- Footer Section -->
